...
LOG_DIR="log/random/silhoutte"
BATCH_TEMPLATE="experiments/ddpm_ss.sh"
JOB_DIR="jobs"
...
for CONFIG in "$CONFIG_DIR"/*; do
    ...
    sed -e "s|{{JOB_NAME}}|$JOB_NAME|" -e "s|{{LOG_PATH}}|\"$LOG_PATH\"|" -e "s|{{CONFIG}}|\"$CONFIG_PATH\"|" "$BATCH_TEMPLATE" > "$BATCH_JOB"
    sbatch "$BATCH_JOB"
