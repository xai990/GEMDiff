...
#     data_filter="replace",
#     gene_selection=None,
#     dge = True, # need to be added to the script_util
# ):
#     """ 
...
#                                         scaler=True,
#                                         filter=data_filter,
#                                         random_selection=Genedifferential(),
#             )
#         else:
#             dataset = CustomGeneDataset(all_files[0],
#                                         all_files[1],
...
#         # logger.info(f"The path of label: {labelpath}")
...
#         #gene, label = self.gene, self.label
#         #gene = self.gene
        

#         out_dict = {}
#         if self.transform:
#             gene = self.transform(self.gene, self.scaler, self.filter)
            
#         if self.target_transform:
#             label = self.target_transform(self.label, self.classes)
...
class CustomGeneDataset(Dataset): 
    def __init__(
        ...
        if dge:
            gene = dge(gene)
        # """ Min-Max scaler"""
        # mins, maxs = gene.min(), gene.max()
        # gene = 2 * (gene - mins) / (maxs - mins) - 1
        self.gene = gene
        self.label = label
...
class GeneDataTransform():
    ...
    def __call__(self, sample, scaler= None, filter = None,):
        ...
        # sample = sample.reshape(B, 128, T * C // 128)
        #return th.from_numpy(sample).float()
        # logger.info(f"The size of sample: {sample.shape} --dataset")
        return sample
